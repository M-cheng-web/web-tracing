/**
 * 会话控制,此会话只和具体的浏览器相关,与业务无关,和业务意义上的登录态没有任何关联,只是用于追踪同一个浏览器上访问页面的动作
 */
import { getCookieByName, uuid } from './util';

const surviveMilliseconds = 1800000; // 存活时长 30minutes

const sessionKey = '_warden_session_id'; // key值固定,一个站点只允许运行一个埋点程序

/**
 * 刷新会话存续期
 */
function refreshSession() {
  const id = getCookieByName(sessionKey) || `s_${uuid()}`;
  const expires = new Date(Date.now() + surviveMilliseconds);
  document.cookie = `${sessionKey}=${id};path=/;max-age=1800;expires=${expires.toUTCString()}`;
  return id;
}

/**
 * 获取id
 */
function getSessionId() {
  return getCookieByName(sessionKey) || refreshSession();
}

refreshSession(); // 初始化

export {
  getSessionId,
  refreshSession,
};
