 ...
 * 主动触发事件
 */
export function traceCustomEvent(eventId, title, params = {}) {
  emit({
...
export default {
  ...
  clickCollection() {
    ...
    document.addEventListener('click', (e) => { // 点击事件
      ...
      _config.title = this.extractTitleByTarget(target);
      _config.eventId = this.extractDataByPath(path);
      _config.params = this.extractParamsByPath(path);
   ...
   * 提取数据事件ID
   */
  extractDataByPath(list = []) {
    /* data-warden-event-id */
    const hasIdEl = this.getElByAttr(list, 'data-warden-event-id');
    if (hasIdEl) return hasIdEl.getAttribute('data-warden-event-id');

    /* title */
    const hasTitleEl = this.getElByAttr(list, 'title');
    if (hasTitleEl) return hasTitleEl.getAttribute('title');

    /* container */
    const container = this.getElByAttr(list, 'data-warden-container');
    if (container) {
  ...
  extractParamsByPath(list = []) {
    ...
    const attrList = container.attributes || [];
    const params = {};
  ...
  extractTitleByTarget(target = {}) {
    ...
    const superTitle = this.getNodeTitle(container);
    ...
    return (!target.hasChildNodes() || tagName.toLowerCase() === 'svg')
      ? this.handleLeafNode(target)
      : this.handleNoLeafNode(target);
  },
  /**
