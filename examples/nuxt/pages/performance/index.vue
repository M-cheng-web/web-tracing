...
function getAllTracingList() {
  $fetch('/getAllTracingList', { params: { eventType: 'performance' } })
    .then(res => {
      const successList = (res as any).data.filter(
        (item: any) => item.eventId === 'resource'
      )
      return $fetch('/getAllTracingList', { params: { eventType: 'error' } })
        .then(res2 => {
          const errorList = (res2 as any).data.filter(
            (item: any) => item.eventId === 'resource'
          )
