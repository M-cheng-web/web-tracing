...
## 待解决
1. event事件里面会带有卸载事件，当unload关闭了还是会触发，去调研一下原因（在form-button的场景里）
2. 并没有显式的暴露上个页面的停留时间，这点我最近考虑会加上去，目前你可以通过两次路由事件的 triggertime 的相差值来判断 - 当为history路由时效果很不好,具体等到后台管理做好了看看怎么样
3. 给插件一个id，在你上传时会拿到这个id的元素的内容值(下拉框，输入框，时间选择器...)
4. 目前不能自己控制传输方式，主要考虑到数据量太大不好控，想个办法
5. nestjs学习
